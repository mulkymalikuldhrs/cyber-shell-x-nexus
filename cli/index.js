...
async function showSystemStatus() {
  ...
  try {
    // Corrected endpoint
    const response = await fetch('http://localhost:5000/api/status', { signal: controller.signal });
    clearTimeout(timeoutId);
    if (response.ok) {
      const status = await response.json();
      spinner.succeed('Server Connection: Active');
      log('info', `AI Status: ${status.message}`);
    } else {
...
async function main() {
    if (process.argv.includes('--manage-tools')) {
        await manageTools();
    } else if (process.argv.includes('--status') || process.argv.includes('status')) {
        await showSystemStatus();
    } else {
