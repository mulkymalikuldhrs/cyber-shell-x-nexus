...
async function handleCommand(command) {
  ...
  if (command === 'help') {
    ...
    while (commandToExecute && commandToExecute.toLowerCase() !== 'finish' && loopCount < maxLoops) {
        ...
        const answer = await new Promise(resolve => rlConfirm.question(`[Loop ${loopCount}/${maxLoops}] Execute command: "${commandToExecute}"? (y/n/quit) `));
        rlConfirm.close();

        if (answer.toLowerCase() === 'n') {
            log('info', 'Skipping command.');
            conversationHistory.push({ role: 'user', parts: `Command "${commandToExecute}" was skipped by the user.` });

            // Ask AI for a different command
            const nextStepSpinner = ora('Asking AI for a different command...').start();
        ...
        if (answer.toLowerCase() === 'quit') {
            log('info', 'Automation quit by user.');
            break;
        }

        log('info', `Executing: ${commandToExecute}`);
